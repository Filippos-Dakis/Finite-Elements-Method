...
% computational method. There is a Rectangular waveguide (infinite length),
% with transverse dimensions (a1 X b1). I solve the eigenvalue problem to 
% obtain the cutoff frequencies and also the modes profile, both for TE and 
% TM modes. The user can chnage several things, such as the number of refinements,
...
close all
clear all
clc                                 %  choose the mode you are interested in calcualting
MODE = 'TM';                        % 'TE' or 'TM'                                     <---------------------
...
a1    = 22.86*mm;                   % (m_units) X-axis dimension   WR-90 -> a = 22.86mm    b = 10.16mm
b1    = 10.16*mm;                   % (m_units) Y-axis dimension
...
Rect = [3; 4; -a1/2; -a1/2; a1/2; a1/2; -b1/2; b1/2; b1/2; -b1/2]; 
gd = [Rect];                        % needed for matlab
ns = char('Rect');                  % names of the boundaries
ns = ns';                           % converting it to column vector
sf = 'Rect';                        % setting the formula for the geometry
...
y_axis  = ['Y-axis (',un,')'];
%% -------------- creating the mesh  --------------------------------------
refin   = 2;                         % choose the number of refinments      <---------------------        
...
[V,D] = eigs(Sff,Tff,N_modes,'smallestabs','tol',1e-10);
D     = diag(D);
fc    = sqrt(D)/(2*pi*sqrt(e_r*e0*mu0));
pnm   = sqrt(D)                       % prints the eigenvalues in command window
%% Converts the geometry into centimeters
dl_plot          = dl;
dl_plot(2:5,1:4) = dl(2:5,1:4)/units;
%% 5. Plots modes profiles TE(E_t = E_x + E_y) and TM(H_t = H_x + H_y)
fig1          = figure();             % To make sure that the saved PDF will be 100% vector      <------ These commands solve the problem of
fig1.Renderer = 'Painters';           % because I hade some problems !! 

if sqrt(N_modes) == fix(sqrt(N_modes))   % nothing special, it just tries to 
    rowz = fix(sqrt(N_modes));           % to make the plot more pretty. :) :)
...
XX1     = linspace(-a1/2,a1/2,1000);                 % points along X-axis
YY1     = linspace(-b1/2,b1/2,1000);                 % points along Y-axis
